	...
	for pair in ticker_pair:
		...
		if price != "price":
			...
			if Company.objects.filter(ticker=ticker).exists():
				...
				obj.save()
	return HttpResponse("Done!")
def populate_initial_list(request):
	for pair in ticker_pair:
		ticker = pair[1]+".OSE"
		price = netfonds_price_yesterday(ticker)
		if price != "price":
			company = Company.objects.create_company(pair[0],ticker,0.0,0.0,0.0)
	return HttpResponse("Done!")